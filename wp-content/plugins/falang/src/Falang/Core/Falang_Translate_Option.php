...
class Falang_Translate_Option {
    ...
    public function __construct( $name, $keys = array(), $args = array() ) {
        ...
        $context = isset( $args['context'] ) ? $args['context'] : 'falang';
        ...
        if ( empty( $args['sanitize_callback'] ) ) {
            add_filter( 'falang_sanitize_string_translation', array( $this, 'sanitize_option' ), 10, 2 );
        } else {
            add_filter( 'falang_sanitize_string_translation', $args['sanitize_callback'], 10, 3 );
        }
    }
    ...
    protected function register_string_recursive( $context, $option, $values, $key ) {
        ...
        if ( is_array( $values ) ) {
            ...
            FString::register_string( $option, $values, $context, true );
        }
    }
    ...
    public function update_option() {
        ...
        //sbou normally not used the save string is done in ajax
        ...
        if ( ! empty( $this->updated_strings ) ) {
            ...
            foreach ( $languages as $language ) {
                ...
                foreach ( $this->updated_strings as $old_string => $string ) {
                    ...
                    //string already translated before for this language
                    if ( $translation !== $old_string ) {
